...
public class PushService extends BackendlessPushService {
    ...
    public boolean onMessage(Context context, Intent intent) {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, Constants.PUSH_WAKE_LOCK);
        wakeLock.acquire();
        WifiManager wm = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
        multicastLock = wm.createMulticastLock(Constants.PUSH_WIFI_LOCK);
        multicastLock.acquire();
        Log.i(Constants.LOG_TAG, intent.toString());
    ...
    private class ReceiveTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... nothing) {
            ...
            if (!intent.hasExtra("action")) {
                // No action specified, so nothing we can do
                return null;
            }
